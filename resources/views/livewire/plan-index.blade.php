        ...
        <div class="space-y-6 max-w-4xl">
            <div>
                ...
                <p class="text-xs text-[var(--ui-muted)] mt-1">
                    <span class="text-indigo-600 font-medium">Master</span> bündeln Instanzen ·
                    <span class="text-emerald-600 font-medium">Instanzen</span> laufen darin ein ·
                    <span class="text-amber-600 font-medium">Detailpläne</span> speisen einzelne Zeilen (Drill-down).
                </p>
            </div>
            ...
            @if($masters->isNotEmpty())
                <section>
                    <h2 class="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-[var(--ui-muted)] mb-2.5">
                        @svg('heroicon-o-square-3-stack-3d','w-3.5 h-3.5 text-indigo-500') Master
                    </h2>
                    <div class="space-y-3">
            ...
            @if($singles->isNotEmpty())
                <section>
                    <h2 class="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-[var(--ui-muted)] mb-2.5">
                        @svg('heroicon-o-chart-bar-square','w-3.5 h-3.5 text-[var(--ui-primary)]') Einzelpläne
                    </h2>
                            ...
                            <a href="{{ route('forecast.plans.show', ['uuid' => $plan->uuid]) }}" wire:navigate
                               class="flex items-center justify-between gap-4 px-4 py-3 hover:bg-[var(--ui-muted-5)] transition-colors">
                                <div class="min-w-0 flex items-center gap-2">
                                    @svg('heroicon-o-chart-bar-square','w-4 h-4 text-[var(--ui-primary)] shrink-0')
                                    <div class="min-w-0">
            ...
            @if($details->isNotEmpty())
                <section>
                    <h2 class="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-[var(--ui-muted)] mb-2.5">
                        ...
                        <span class="font-normal normal-case tracking-normal text-[var(--ui-muted)]/70">— Bausteine, normal per Drill-down erreichbar</span>
                    </h2>
