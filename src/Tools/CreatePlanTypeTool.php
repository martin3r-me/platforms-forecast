                ...
                'rows' => [
                    ...
                    'items' => [
                        ...
                        'properties' => [
                            ...
                            'unit' => ['type' => 'string', 'description' => 'Einheit-Code: EUR, KEUR, H, MIN, FTE, PCS, PCT (berechnetes %, Wert 0–100), FAKTOR (eingegebener Anteil 0–1, Anzeige als %, wird multipliziert nicht summiert).'],
                            ...
                            'time_agg' => ['type' => 'string', 'enum' => ['flow', 'stock', 'stock_open', 'avg', 'recompute', 'wavg'], 'description' => 'Zeit-Aggregation: flow (Default, summiert über die Zeit — Umsatz/Kosten) · stock (Bestand, Schlusswert = jüngster Teilzeitraum — Personal/Bestand/Cash/Schulden; Q1=März, nicht Σ) · stock_open (Eröffnungswert) · avg (Durchschnitt) · wavg (gewichteter Ø über die Zeit, braucht weight_by — z. B. Ø-Preis gewichtet nach Menge: Jahr = Σ(Preis×Menge)÷Σ(Menge)). '
                                .'Bei Ausdruck-Zeilen (config.expr): flow/avg/stock rechnen NUR auf der feinsten Ebene und rollen hoch (z. B. Staffel-Provision: Jahr = Σ der Monats-Provisionen). recompute = Ausdruck auf JEDER Ebene neu rechnen — richtig für Quoten/Margen (Jahres-Marge = Jahr-DB ÷ Jahr-Umsatz).'],
                            'weight_by' => ['type' => 'string', 'description' => 'Nur bei time_agg=wavg: row_key der Gewichts-Zeile (z. B. "menge"), nach der über die Zeit gemittelt wird.'],
